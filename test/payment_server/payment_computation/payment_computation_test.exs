defmodule PaymentServer.PaymentComputationTest do
  ...
  describe "&get_total_amount/2" do
    test "returns sum of amount in wallets" do
      ...
      assert PaymentComputation.get_total_amount([{"USD", 200}, {"EUR", 300}], "USD") ===
               {:ok, 458.0}
    end
  end
  ...
  describe "&transfer_money/5" do
    test "transfers money from one wallet to another" do
      ...
      assert {:ok, [sender_wallet, receiver_wallet]} =
  ...
  defp create_wallets() do
    ...
    {:ok, wallet1} =
      Payments.create_wallet(%{amount: 200, user_id: user1.id, currency_id: currency.id})

    {:ok, wallet2} =
      Payments.create_wallet(%{amount: 100, user_id: user2.id, currency_id: currency.id})
